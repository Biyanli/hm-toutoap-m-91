<template>
  <div class="comment">
    <!-- 列表 上拉加载功能 loading是否开启加载状态 finished是否已经全部加载完成 -->
    <!-- van-list组件 可以完成上拉加载动作 -->
    ...
    <van-action-sheet @closed="reply.commentId=null"  v-model="showReply" :round="false" class="reply_dialog" title="回复评论">
      <!-- 回复组件 -->
      <!-- immediate-check 是否在初始化时立即执行滚动位置检查 关闭了主动检查 第一次不会主动的去调用load事件对应的方法了 -->
...
<script>
import { getComments, commentOrReply } from '@/api/article' // 引入封装的获取评论的方法 回复评论的方法
export default {
  data () {
    return {
      // 上拉加载中
      ...
      offset: null, // 表示分页依据 如果为空,表示从第一页开始 获取文章评论的评论的分页依据 a
  ...
  methods: {
    ...
    openReply (commentId) {
      ...
      // 开始加载第一页的数据了
      this.getReply() // 开始调用第一页的数据 前面的状态已经设置完毕
    },
    ...
    // 加载逻辑
      ...
      this.reply.list.push(...data.results) // 将评论的评论加载到数据尾部
      // 拿到的data只是请求的第一页的数据
    ...
    // 加载评论数据
      let data = await getComments({
        ...
        source: this.$route.query.articleId // 表示获取文章的id
      })
      this.comments.push(...data.results) // 将数据添加到当前评论列表的尾部
      this.loading = false // 关闭正在上拉加载状态
      this.finished = data.last_id === data.end_id // 如果当前页id等于整个评论最后一个id 表示 我们已经请求完了所有的数据
      if (!this.finished) {
